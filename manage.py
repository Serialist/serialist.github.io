...
class BlogManager:
    ...
    def add_or_update(self, fpath):
        ...
        with open(
            os.path.join(self.args.output, f"{item['id']}.md"), "w", encoding="utf-8"
        ) as f:
    ...
    def add_logic(self):
        targets = (
            self.args.input if isinstance(self.args.input, list) else [self.args.input]
        )
        all_files = []
        for target in targets:
            if os.path.isdir(target):
                for r, _, fs in (
                    ...
                    else [(target, [], os.listdir(target))]
                ):
                    all_files.extend(
                        [os.path.join(r, f) for f in fs if f.endswith(".md")]
                    )
    ...
    def create_article(self, filename):
        ...
        input_dir = (
            self.args.input[0]
            if (isinstance(self.args.input, list) and os.path.isdir(self.args.input[0]))
            else "input"
        )
...
class ArticleTUI:
    ...
    def edit_string(self, stdscr, prompt, initial_text=""):
        ...
        while True:
            ...
            try:
                ...
                target_x = get_display_width(display_prefix) + get_display_width(
                    "".join(s[:pos])
                )
    ...
    def draw(self, stdscr):
        ...
        for i in range(h - 2):
            ...
            try:
                stdscr.addstr(
                    i, 0, f"{char} | {title_text}{padding} | {c_d} | {m_d}"[: w - 1]
                )
            except:
                pass
        ...
        try:
            ...
            for i, (name, _) in enumerate(opts):
                if x_offset + get_display_width(name) >= w:
                    break
                if i == self.opt_idx:
                    stdscr.attron(curses.A_REVERSE)
    ...
    def handle_action(self, stdscr, key):
        if key == "n":
            ...
            if not self.selected:
                ...
                self.menu_state, self.opt_idx, self.status = "ATTR", 0, "Edit field..."
        elif key == "2":
            if not self.selected:
                self.status = "Nothing selected!"
            else:
                self.status, self.menu_state, self.opt_idx, self.pending_action = (
                    f"CONFIRM: Delete {len(self.selected)} items?",
                    "CONFIRM",
                    1,
                    self.do_delete,
                )
        ...
        while True:
            ...
            if ch == 27:  # ESC
                if self.menu_state != "MAIN":
                    self.menu_state, self.opt_idx, self.status = "MAIN", 0, "Ready"
                else:
                    break
                ...
                if self.menu_state == "CONFIRM":
                    if self.menus["CONFIRM"][self.opt_idx][1] == "y":
                        self.pending_action()
                ...
                if ord("1") <= ch <= ord("6"):
                    self._perform_attr_edit(
                        stdscr, self.menus["ATTR"][ch - ord("1")][1]
                    )
    ...
    def _perform_attr_edit(self, stdscr, field):
        ...
        art = next(a for a in self.mgr.articles if a["id"] == uid)
        curr = (
            ",".join(art.get(field, [])) if field == "tag" else str(art.get(field, ""))
        )
        new_v = self.edit_string(stdscr, f"Edit {field}", curr)
        if new_v is not None:
            art[field] = (
                [x.strip() for x in new_v.split(",")] if field == "tag" else new_v
            )
            self.mgr.save_json()
            self.status = "Updated."
    ...
    def _handle_main_input(self, stdscr, ch, h):
        ...
        if ch == curses.KEY_UP:
            ...
            arts = self.get_list()
            if self.cursor < len(arts) - 1:
                self.cursor += 1
            if self.cursor >= self.offset + (h - 2):
                self.offset += 1
        elif ch == 26:  # Ctrl+Z
            if UNDO_CACHE:
...
def main():
    ...
    parser.add_argument(
        "-i", "--input", nargs="+", default=conf.get("input_path", ["input"])
    )
    ...
    for attr, d_val in [
        ...
        parser.add_argument(
            f"--{attr if attr != 'title' else 'default_title'}",
            default=conf.get(attr, d_val),
