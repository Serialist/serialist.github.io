...
def manage():
    ...
    parser.add_argument("--list", default=CONFIG["JSON_OUT"])
    parser.add_argument("--assets", default=CONFIG["ASSETS_DIR"])
    ...
    for fpath in files:
        ...
        # 写入输出目录（不带 YAML 头部）
        with open(
            os.path.join(CONFIG["OUTPUT_DIR"], f"{item['id']}.md"),
            "w",
            encoding="utf-8",
        ) as f:
